...
describe('drug-events-service', function () {
  ...
  it('calculates reaction outcomes', function () {
    var params = {'search': 'patient.drug.openfda.brand_name:Synthroid',
                  'count': 'patient.reaction.reactionmeddrapt.exact',
                  'limit': '20'};
    $httpBackend.expectGET('https://api.fda.gov/drug/event.json?search=patient.drug.openfda.brand_name:Synthroid+AND+patient.reaction.reactionmeddrapt:"NAUSEA"&count=patient.reaction.reactionmeddrapt.exact').respond({results: [{term: 'NAUSEA', count: 83}]});
    $httpBackend.expectGET('https://api.fda.gov/drug/event.json?search=patient.drug.openfda.brand_name:Synthroid+AND+patient.reaction.reactionmeddrapt:"DYSPNOEA"&count=patient.reaction.reactionmeddrapt.exact').respond({results: [{term: 'DYSPNOEA', count: 72}]});
    ...
    expect(DrugEventsService.reactionResults[0].outcomes[0].term).toBe('NAUSEA');
    expect(DrugEventsService.reactionResults[0].outcomes[0].count).toBe(83);
    expect(DrugEventsService.reactionResults[1].outcomes[0].term).toBe('DYSPNOEA');
    expect(DrugEventsService.reactionResults[1].outcomes[0].count).toBe(72);
  });
  ...
  it('sets outcome to 6 when a 404 is returned', function () {
    ...
    expect(DrugEventsService.reactionResults[0].outcomes[0].term).toBe(6);
    expect(DrugEventsService.reactionResults[0].outcomes[0].count).toBe(20);
  });
});
